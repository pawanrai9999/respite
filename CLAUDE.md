...
**Current state:** Phases 1 (settings foundation), 2 (timer engine), and 3 (process model & daemon) are complete. The "Hello World" template has been replaced with an Adwaita preferences window whose controls are bound two-way to GSettings with input constraints enforced, and a UI-agnostic `RespiteTimer` GObject (`src/respite-timer.c`) drives the work/break countdown via a monotonic clock, emitting `tick`/`state-changed`/`break-started`/`break-ended` signals (exercised by the temporary `--debug-timer` console mode in `main.c`). `RespiteApplication` owns a single shared `RespiteTimer` created in `startup`; the app uses `G_APPLICATION_HANDLES_COMMAND_LINE` so a `--daemon` invocation runs headless (takes a `g_application_hold`, starts the timer, no window) while any other invocation presents/raises the single settings window, and the quit/shutdown path stops the timer and releases the hold. The break overlay, postpone mechanism, and autostart are not implemented yet (see `PLAN.md` for the phased roadmap and current status). The README's "Behavior in Detail" and "Roadmap" sections describe the intended design.
